...
import { User, setEmail, setLoading, setSecretInfo, setToken } from "../slices/AuthSlice"
...
export const ConfirmUser = (payload: User) => {
    return async (dispatch: AppDispatch) => {
        try {
            dispatch(setLoading(true))
            const authPost = await axios.post(`${api}/api/user/confirm`, { email: payload.email, code: payload.code })
